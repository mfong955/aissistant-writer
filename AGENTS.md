...
- **Editor formatting toolbar** (`src/components/editor/markdown-toolbar.tsx`, first tier of a small→medium→big richness plan) — buttons for headings, bold, italic, underline, strike, inline code, code blocks, blockquote, bullet/ordered lists, horizontal rule, and links, wired into the *real* `markdown-editor.tsx`. Since there's no WYSIWYG layer, each button is a pure text transform on the raw markdown at the cursor/selection (`src/lib/markdown-toolbar-actions.ts` — wrap/toggle/insert functions, editor-agnostic and unit-testable) rather than an editor command; selection is restored after each edit so typing can continue immediately. Buttons don't show an "active" state (e.g. Bold doesn't highlight while the cursor sits in bold text) — that would need the component to track live textarea selection reactively, deferred as a nice-to-have. Multi-line list-prefix toggling has one known cosmetic imprecision: the restored selection can be off by the prefix's length on the first line after toggling — the saved text is always correct, only the immediate post-click highlight can be slightly off.
