...
**What's next:**
1. **Apply flow** — the reviewable bridge from canvas/import into real Canon/Manuscript/Unsorted entities. Fully specced in `docs/apply-flow.md`: a `propose_plan` tool (mirrors `ask_question` — pure surface-and-pause, halts the round loop), a plain `POST /api/plans/apply` route that executes accepted items through the existing entity CRUD (no model round-trip needed), a review card in chat, and a canvas "Apply to Project" button wired through the existing `aissistant:add-to-chat`-style event bridge. This is what makes canvas mode and import staging (items below) both actually reach the project instead of dead-ending. **Migration `005_canvas_mode.sql` still needs to be applied to Supabase** before canvas mode works end to end at all.
2. **Import staging** — subsumed into item 1's scope (`docs/apply-flow.md` §6): the extraction UI already exists (paperclip attach in chat), what's missing is routing organize-from-import through `propose_plan` instead of filing directly. No separate build.
3. **BYOK activation path** — reduce the copy-paste-a-key barrier via OpenRouter's OAuth PKCE connect flow. Confirmed feasible and documented: `GET https://openrouter.ai/auth?callback_url=...&code_challenge=...&code_challenge_method=S256` → user authorizes → redirect back with `code` → `POST https://openrouter.ai/api/v1/auth/keys` with `{ code, code_verifier, code_challenge_method }` returns a provisioned key, stored exactly like a manually-pasted one. No app registration required. Docs: `openrouter.ai/docs/guides/overview/auth/oauth`.
4. Editor "big" tier — footnotes/cross-references, deferred by Matthew's choice while equations get scoped separately (see "Also under discussion" below)
...
| Canvas mode (visual plot/story mapping) | `docs/canvas-mode.md` — data model, UI, and AI tools built; apply-to-project still pending on `docs/apply-flow.md` |
| Apply flow (canvas/import → real project, reviewed) | `docs/apply-flow.md` — specced, not yet built |
