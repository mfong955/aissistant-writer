...
**What's next:**
1. **Canvas AI tools** — `read_canvas`/`update_canvas`, wired into the chat context builder. Canvas mode UI is built; **migration `005_canvas_mode.sql` still needs to be applied to Supabase** before any of it actually works end to end.
2. **Canvas apply-to-project flow** — plan generation + per-item review dialog, reusing the rename-sync review UI pattern. Depends on item 1.
...
5. Editor "big" tier — footnotes/cross-references, deferred by Matthew's choice while equations get scoped separately (see "Also under discussion" below)
